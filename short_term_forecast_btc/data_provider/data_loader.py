...
class BTC_Dataset(Dataset):
    ...
    def __read_data__(self):
        # Load the CSV file containing Bitcoin data
    ...
    def __getitem__(self, index):
        ...
        insample = np.zeros((self.seq_len, len(self.features)))  # Multivariate input
        insample_mask = np.zeros((self.seq_len, 1))  # Univariate mask
        outsample = np.zeros((self.pred_len + self.label_len, len(self.features)))  # Multivariate output
        outsample_mask = np.zeros((self.pred_len + self.label_len, 1))  # Univariate mask for outsample

        # Get the sampled time series data
        sampled_timeseries = self.data  # Using self.data instead of self.timeseries

        # Calculate the cut point for splitting the time series into insample and outsample windows
        cut_point = np.random.randint(
            low=max(1, len(sampled_timeseries) - self.seq_len - self.pred_len),
            high=len(sampled_timeseries) - self.pred_len,
            size=1
        )[0]

        # Get the insample window
        insample_window = sampled_timeseries[max(0, cut_point - self.seq_len):cut_point]
        insample[-len(insample_window):, :] = insample_window  # Multivariate input, fill insample with values
        insample_mask[-len(insample_window):, 0] = 1.0  # Set mask for the insample

        # Get the outsample window
        outsample_window = sampled_timeseries[cut_point:cut_point + self.pred_len + self.label_len]
        outsample[:len(outsample_window), :] = outsample_window  # Multivariate output, fill outsample with values
        outsample_mask[:len(outsample_window), 0] = 1.0  # Set mask for the outsample

        # Return the insample, outsample, and corresponding masks
